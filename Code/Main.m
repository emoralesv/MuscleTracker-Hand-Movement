...
%% Reading the data
clear all
ds{1} = tabularTextDatastore("BIOPAC FOLDER","FileExtensions",".txt"); % reading BIOPACK data
ds{2} = tabularTextDatastore("MIOTRACKER FOLDER","FileExtensions",".txt"); % reading MIOTRACKER data
...
trainWith = [2]; % databases to experiment, [1] train with database 1, [2] train with database 2, [1 2] train with the two databases for cross database experiments
numfeatures = 60;
...
mn(1:size(ds,2)) = min(n); % balancing the databases
%mn = n; % Uncomment this to avoid class balancing
% index to calculate results by database

%perm =  zeros(mn,size(ds,2));
perm = cell(1,size(ds,2));
for p = 1: size(ds,2), perm{p} = randperm(n(p),mn(p)); end
...
for  spatialSamples = 1
    ...
    for i = 1: size(data.Files,1)
        ...
        for samples = 1: delta:  spatialSamples*delta
            ...
            %writematrix([cc1 cc2],strcat(" DESTINATION FOLDER",name,"_",num2str(cc),ext));
